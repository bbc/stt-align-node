...
function interpolate(wordObjectsList){
    ...
    wordObjectsList.forEach((word, index)=>{
        //TODO:  ISSUE, what to do if the transcript is missing more then one 
        // consecutive words
        // TODO: look into SRT interpolation code from popcorn js 
        // https://github.com/pietrop/srtParserComposer/blob/master/srtJsonToWordLinesJson.js
        ...
        if(word.start === null){
            ...
            // first element
                ...
                // console.log(word, previousWord.end,nextWord.start)
                resultWordsList.push({
                    start: nextWord.start - calculateWordDuration(word.word), 
                    end: nextWord.start,
                    word: word.word,
                })
            }
            // last element 
            else if(index ===(wordObjectsList.length-1)){
                const previousWord = wordObjectsList[index-1];
                // const nextWord = wordObjectsList[index+1];
                // console.log(word, previousWord.end,nextWord.start)
                resultWordsList.push({
                    start: previousWord.end, 
                    end: previousWord.end + calculateWordDuration(word.word),
                    word: word.word,
                })
            }
        }
