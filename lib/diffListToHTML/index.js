const fs = require('fs');
// const diffs = require('../../sample/output/modbydick-diffs.json');
const diffs = require('../../sample/output/ted-talk-kate-diffs.json');
...
htmlResult.push(`
equal: ${diffs.count.equal} <br>
insert: ${diffs.count.insert}<br>
delete: ${diffs.count.delete}<br>
replace: ${diffs.count.replace}<br>
Base Text Total Word Count: ${diffs.count.baseTextTotalWordCount}
<hr>
`)

htmlResult.push(`
Video <input class='videoInput' type="file" name="video" accept="video/*">
<br>
...
diffs.diffsList.forEach(element => {
...
// fs.writeFileSync('./sample/output/modbydick-diffs.html',htmlResult.join(' ') )

fs.writeFileSync('./sample/output/ted-talk-kate-diffs.html',htmlResult)